...
import { ProcessData, ProcessStatus, ProcessSuggestionRequest } from "@/interfaces/processes";
...
export const ProcessSelectionDrawer = ({
  ...
  const { data: suggestedProcesses, isLoading } = useQuery({
    queryKey: ["template-suggestion", processData.name, processData.type, processData.output_type, processData.project_id],
    queryFn: async () => {
      ...
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-auto">
          <div className="flex flex-col gap-2">
            {suggestedProcesses?.map((process: ProcessData, index: number) => (
              <SelectableAccordion
                key={`process-suggest-${index}`}
                title={process.name && process.name?.length > 0 ? process.name : "extract"}
                isSelected={selectedTemplate !== null && selectedTemplate.id === process.id}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-100 p-4 rounded-lg">
                      ...
                      {process.completed_at ? (
                        <DateLabel
                          dateString={process.completed_at}
                        />
                      ) : (
